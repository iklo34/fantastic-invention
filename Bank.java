...
public class Bank {
    public static void main(String[] args) {
        ...
        f222222.setDepositWithdrawStrategy(
                new DifferentCurrencyStrategy()
        );
...
interface DepositWithdrawStrategy {
    void execute(BankAccount account, double amount,Currency currencyTar);
}
class DifferentCurrencyStrategy implements DepositWithdrawStrategy {
    @Override
    public void execute(BankAccount account, double amount,Currency currencyTar) {
        Currency currency = account.getCurrency();
        account.setBalance(account.getBalance() + (amount * currencyTar.Ex_rate) / currency.getExchangeRate());
    }
}
...
class BankAccount {
    ...
    public BankAccount(User owner, Currency currency, double interestRate, double creditLimit, double balance) {
        ...
        this.balance = balance;
        this.depositWithdrawStrategy = new DifferentCurrencyStrategy();
    }
    ...
    public void DepositMoney(double amount, Currency currency1) {
        ...
        depositWithdrawStrategy.execute(this,amount,currency1);
    ...
    public void WithdrawMoney(double amount, Currency currency1) {
        ...
        if (balance - withdrawalAmount >= creditLimit) {
            depositWithdrawStrategy.execute(this, -withdrawalAmount,currency1 );
